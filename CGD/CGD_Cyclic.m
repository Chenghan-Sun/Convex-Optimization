...
%CGD method with cyclic rule and fixed step size for solving Lasso Problem 
clear all, clc
...
%CGD method with cyclic rule terminates when norm(xk-xs)/norm(xs) smaller than given epsi = 10^-3
...
figure 
plot(1:k,cr)
title('Cyclic CGD method satisfied the termination criteria using epsilon=10^-3');
xlabel('Number of iteration');
ylabel('Termination Criteria');

figure
plot(log(1:k),log(error))
title('Log-log plot of function value error of Cyclic CGD method using epsilon=10^-3');
